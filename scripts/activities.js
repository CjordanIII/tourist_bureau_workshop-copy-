...
categories.unshift("Select an Activity");
categories.forEach((category) => {
  let option = document.createElement("option");
  option.textContent = category;
  option.value = category;
  categorys.appendChild(option);
  categorys.firstChild.value = "";
});
// intrate though
console.log(categorys.childNodes[1]);

const option = document.querySelector("select > option");

categorys.addEventListener("change", (e) => {
  const data = activities.filter(
    (activite) => activite.category === e.target.value
  );

  // options
  if (e.target.value) {
    const activeSelecter = document.querySelector("#activities");
    activeSelecter.style.display = "block";
    const activites1 = document.querySelector("#activities");
    data.forEach((activite) => {
      let childActivites = document.createElement("option");
      childActivites.textContent = activite.name;
      childActivites.value = activite.id;
      activites1.appendChild(childActivites);
    });
  }
});
